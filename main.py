...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, re, u, v, layers):

        X = np.concatenate([x, y, re], 1)
        ...
        self.re_tf = tf.placeholder(tf.float32, shape=[None, self.re.shape[1]])
    ...
    def net_NS(self, x, y, re):
        # lambda_1 = self.lambda_1
        # lambda_2 = self.lambda_2

        psi_and_p = self.neural_net(tf.concat([x, y, re], 1), self.weights, self.biases)
        psi = psi_and_p[:, 0:1]
        p = psi_and_p[:, 1:2]
        ...
        f_u = re * ((u * u_x + v * u_y) + p_x) - (u_xx + u_yy)
        f_v = re * ((u * v_x + v * v_y) + p_y) - (v_xx + v_yy)
    ...
    def train(self, nIter):
        ...
        tf_dict = {self.x_tf: self.x, self.y_tf: self.y, self.re_tf: self.re,
    ...
    def predict(self, x_star, y_star, re_star):

        tf_dict = {self.x_tf: x_star, self.y_tf: y_star, self.re_tf: re_star}
